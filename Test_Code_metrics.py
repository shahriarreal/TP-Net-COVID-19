...
if __name__ == '__main__':
    ...
    for cat in range(classes):
        target_names.append('class_' + str(cat))
    predicted_X = np.argmax(eval_result, axis=1)
    one_hot_results = np.zeros_like(eval_result)
    for i, item in enumerate(predicted_X):
        one_hot_results[i][item] = 1
    print(classification_report(y_val, one_hot_results, target_names=target_names, digits=6))